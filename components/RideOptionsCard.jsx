...
const RideOptionsCard = () => {
  ...
  return (
    <SafeAreaView style={tw`bg-white flex-grow`}>
        <View>
            <TouchableOpacity onPress={() => navigation.navigate("NavigateCard")} style={tw`absolute pl-3 pt-2 z-50 rounded-full`}>
            ...
            renderItem={({ item: { id, title, multiplier, image }, item }) => (
                <TouchableOpacity style={tw`flex-row justify-between items-center px-10 ${id === selected?.id && "bg-gray-200"}`} onPress={() => setSelected(item)}>
                    ...
                    <View style={tw`-ml-6`}>
                        ...
                        <Text style={tw`text-xs text-gray-500 px-2`}>{travelTimeInformation?.duration?.text} travel time</Text>
                    </View>
